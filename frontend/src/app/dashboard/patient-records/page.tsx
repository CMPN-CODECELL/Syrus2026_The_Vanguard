...
import {
    ...
    Calendar, Clock, Video, MapPin, ChevronRight, RefreshCw, CalendarSearch
} from 'lucide-react'
...
export default function PatientRecordsPage() {
    ...
    const filtered = records.filter(r => {
        ...
        const matchesDate = !dateFilter || r.scheduled_time?.startsWith(dateFilter)
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="relative">
                    <CalendarSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                    <input
                        type="date"
                        value={dateFilter}
                        onChange={e => setDateFilter(e.target.value)}
                        className="pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                    />
                    {dateFilter && (
                        <button
